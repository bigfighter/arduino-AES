#include "aes.h"

#if false

// lookup table version:
// faster, but uses more memory.

const unsigned char sbox[256] = {
	,
};

#define rj_sbox(x) sbox[x]

#else

// on-the-fly computation:
// slower, but uses less memory.

unsigned char gf_alog(unsigned char x) {
	unsigned char atb = 1, z;

	while (x--) {
		z = atb;
		atb <<= 1;
		if (z & 0x80) {
			atb^= 0x1b;
		}
		atb ^= z;
	}

	return atb;
}

unsigned char gf_log(unsigned char x) {
	unsigned char atb = 1, i = 0, z;

	do {
		if (atb == x) {
			break;
		}
		z = atb;
		atb <<= 1;
		if (z & 0x80) {
			atb^= 0x1b;
		}
		atb ^= z;
	} while (++i > 0);

	return i;
}

unsigned char rj_sbox(unsigned char x) {
	unsigned char y, sb;

	sb = y = x ? gf_alog(255 - gf_log(x)) : 0;
	y = (y<<1) | (y>>7);
	sb ^= y;
	y = (y<<1) | (y>>7);
	sb ^= y;

	y = (y<<1) | (y>>7);
	sb ^= y;
	y = (y<<1) | (y>>7);
	sb ^= y;

	return (sb ^ 0x63);
}

#endif

unsigned char rj_xtime(unsigned char x) {
	return (x & 0x80) ? ((x << 1) ^ 0x1b) : (x << 1);
}

void subBytes(unsigned char* buf) {
	for (unsigned char i = 0; i < 16; i++) {
		buf[i] = rj_sbox(buf[i]);
	}
}

void addRoundKey(unsigned char* buf, unsigned char* key) {
	for (unsigned char i = 0; i < 16; i++) {
		buf[i] ^= key[i];
	}
}

void addRoundKey_cpy(unsigned char* buf, unsigned char* key, unsigned char* cpk) {
	for (unsigned char i = 0; i < 16; i++) {
		buf[i] ^= (cpk[i] = key[i]);
		cpk[16+i] = key[16+i];
	}
}

void shiftRows(unsigned char* buf) {
	unsigned char i;

	i = buf[1];
	buf[1] = buf[5];
	buf[5] = buf[9];
	buf[9] = buf[13];
	buf[13] = i;

	i = buf[10];
	buf[10] = buf[2];
	buf[2] = i;

	i = buf[3];
	buf[3] = buf[15];
	buf[15] = buf[11];
	buf[11] = buf[7];
	buf[7] = i;

	i = buf[14];
	buf[14] = buf[6];
	buf[6]  = i;
}

void mixColumns(unsigned char* buf) {
	unsigned char i, a, b, c, d, e;

	for (i = 0; i < 16; i += 4) {
		a = buf[i+0];
		b = buf[i+1];
		c = buf[i+2];
		d = buf[i+3];
		e = a ^ b ^ c ^ d;
		buf[i+0] ^= e ^ rj_xtime(a^b);
		buf[i+1] ^= e ^ rj_xtime(b^c);
		buf[i+2] ^= e ^ rj_xtime(c^d);
		buf[i+3] ^= e ^ rj_xtime(d^a);
	}
}

void expandEncKey(unsigned char* key, unsigned char* rc) {
	unsigned char i;

	key[0] ^= rj_sbox(key[29]) ^ (*rc);
	key[1] ^= rj_sbox(key[30]);
	key[2] ^= rj_sbox(key[31]);
	key[3] ^= rj_sbox(key[28]);

	*rc = ((*rc<<1) ^ (((*rc>>7) & 1) * 0x1b));

	for (i = 4; i < 16; i += 4) {
		key[i+0] ^= key[i-4];
		key[i+1] ^= key[i-3];
		key[i+2] ^= key[i-2];
		key[i+3] ^= key[i-1];
	}
	key[16] ^= rj_sbox(key[12]);
	key[17] ^= rj_sbox(key[13]);
	key[18] ^= rj_sbox(key[14]);
	key[19] ^= rj_sbox(key[15]);

	for (i = 20; i < 32; i += 4) {
		key[i+0] ^= key[i-4];
		key[i+1] ^= key[i-3];
		key[i+2] ^= key[i-2];
		key[i+3] ^= key[i-1];
	}
}

void AES::ecb(unsigned char* buf) {
	unsigned char i, rcon;

	addRoundKey_cpy(buf, enckey, key);
	for (i = 1, rcon = 1; i < 14; ++i) {
		subBytes(buf);
		shiftRows(buf);
		mixColumns(buf);
		if (i & 1) {
			addRoundKey(buf, &key[16]);
		} else {
			expandEncKey(key, &rcon), addRoundKey(buf, key);
		}
	}
	subBytes(buf);
	shiftRows(buf);
	expandEncKey(key, &rcon);
	addRoundKey(buf, key);
}

void AES::reset() {
	unsigned char i;

	for (i = 0; i < 32; i++) {
		enckey[i] = key[i];
	}
	otpPos = 0;
}

void incrementCounter(unsigned char* counter) {
	unsigned char i, j;

	for (i = 15; i >= 0; i--) {
		for (j = 0; j < 8; j++) {
			if ((counter[i] & (1 << j)) == 0) {
				counter[i] |= (1 << j);
				counter[i] &= (0xFF << j);
				for (++i; i < 16; i++) {
					counter[i] = 0;
				}
				return;
			}
		}
	}
}

unsigned char AES::getNextCtrOtp() {
	unsigned char i;

	if (otpPos == 0) {
		for (i = 0; i < 16; i++) {
			otp[i] = counter[i];
		}
		ecb(otp);

		incrementCounter(counter);
	}

	i = otp[otpPos];
	otpPos = (otpPos + 1) % 16;
	return i;
}

void AES::process(unsigned char* buffer) {
	*buffer ^= getNextCtrOtp();
}
